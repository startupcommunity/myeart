...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  watch: {
    userProfile: function userProfile(val) {
      if (val.id) {
        this.haveAChargingMethod();
      }
    }
  ...
  methods: {
    ...
    haveAChargingMethod: function haveAChargingMethod() {
      ...
      this.axios.get(this.ep.user.getUserChargeMethods + this.userProfile.id).then(function (resp) {
        if (resp.data.length === 0) {
          _this.hasPaymentMethod = false; // this.formIsValid = false;
    ...
    saveArtwork: function saveArtwork() {
      ...
      var state = this.isDraft === 1 ? !this.hasPaymentMethod ? 5 : 1 : 3; // const in_pause = !this.hasPaymentMethod && this.isDraft === 1 ? 1 : 0;
      ...
      data.append("state", state); // data.append("in_pause", in_pause);
      ...
      this.axios.post(this.ep.artworks.save, data, {
        ...
        if (resp.status === 200) {
          ...
          if (type_artwork == 2) {
            _this2.$router.push("/colectivos/perfil/".concat(_this2.collectiveId, "/artwork"));
          }
        }
      })["catch"](function (error) {
